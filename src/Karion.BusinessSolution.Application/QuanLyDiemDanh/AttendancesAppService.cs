    ...
    public class AttendancesAppService : BusinessSolutionAppServiceBase, IAttendancesAppService
    {
        ...
        public async Task<BaoCaoTongHopResultDto> BaoCaoTongHop(BaoCaoTheoKyInputDto input)
        {
            ...
            var thongTinDonVi = await _thongTinDonViRepository.GetAll()
                .Where(x => x.Key == "GioLamViecSang" || x.Key == "GioLamViecChieu" || x.Key == "ThuBay" || x.Key == "ChuNhat")
                .ToListAsync();
            ...
            var allDates = Enumerable.Range(0, (toDate.Date - fromDate.Date).Days + 1)
                .Select(offset => fromDate.Date.AddDays(offset)).ToList();
            ...
            foreach (var nv in nhanViens)
            {
                ...
                foreach (var date in allDates)
                {
                    ...
                    var logsTrongNgay = records.Where(x => x.CheckIn.Date == date).ToList();

                    if (isWorkingDay)
                    {
                        if (logsTrongNgay.Any())
                        {
                            tongLam++;
                            var times = logsTrongNgay.Select(x => x.CheckIn.TimeOfDay).OrderBy(x => x).ToList();
